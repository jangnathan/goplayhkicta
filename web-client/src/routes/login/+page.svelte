<script>
  ...
  async function handleSignIn(e) {
    ...
    try {
      ...
      window.location.href = '/app'; // Redirect to main dashboard
    } catch (error) {
      console.error('Sign in error:', error.message);
      // Map cryptic Firebase errors to user-friendly messages if desired
...
<div class="page-container">
  <div class="auth-card">
    <div class="card-header">
      <h1>Welcome back</h1>
      <p>Sign in to your account to find active games nearby.</p>
    </div>
    ...
    <button type="button" class="nsg-button" onclick={handleGoogleSignIn} disabled={loading}>
      ...
      <span class="nsg-text">Sign in with Google</span>
    </button>
    ...
    <div class="card-footer">
      <p>Don't have an account yet? <a href="/register" class="link">Sign Up</a>.</p>
    </div>
  </div>
</div>

<style>
  /* Sharing design foundations with registration card */
  ...
  /* Corporate Google Widget Specification Layout */
  .nsg-button {
  ...
  .nsg-button:disabled { opacity: 0.5; cursor: not-allowed; box-shadow: none; }
  .nsg-icon-wrapper { display: flex; align-items: center; justify-content: center; width: 18px; height: 18px; }
  .nsg-icon { width: 100%; height: 100%; }
  .nsg-text { font-weight: 600; font-size: 15px; color: #1f1f1f; }
</style>